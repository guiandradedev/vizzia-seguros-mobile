...
export async function loginApi(credentials: Credentials): Promise<void> {
  // await delay(400);
  ...
  // return user;
  const user: AxiosResponse<AuthResponse> = await axios.post('/auth/login', { email: credentials.email, password: credentials.password })
  console.log(user)
}
